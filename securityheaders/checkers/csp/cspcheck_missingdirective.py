    ...
    def check(self):
        ...
        if csp.parsedstring and csp.directive.DEFAULT_SRC in csp.parsedstring:
            ...
            if not csp.directive.OBJECT_SRC in csp.parsedstring and (not csp.keyword.NONE in defaultSrcValues or not str(csp.keyword.NONE) in defaultSrcValues):
                findings.append(Finding(csp.headerkey, FindingType.MISSING_DIRECTIVES, 'We recommend restricting object-src to \'none\'.',FindingSeverity.HIGH_MAYBE, csp.directive.OBJECT_SRC))
        ...
        for directive in directivesCausingXss:
            if not csp.parsedstring or not directive in csp.parsedstring:
                ...
                if directive == csp.directive.OBJECT_SRC:
                    description = 'Missing object-src allows the injection of plugins which can execute JavaScript. We recommend setting it to \'none\'.'
                    ...
                    if not csp.policyHasScriptNonces() and not csp.policyHasScriptHashes() and csp.policyHasStrictDynamic():
                        continue
                    description = 'Missing base-uri allows the injection of base tags. They can be used to set the base URL for all relative (script) URLs to an attacker controlled domain. We recommend setting it to \'none\' or \'self\'.'
